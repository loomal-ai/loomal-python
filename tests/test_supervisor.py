...
import pytest
import respx
from loomal import LoomalSupervisor, AsyncLoomalSupervisor
from loomal.types import CreateIdentityResponse, IdentityDetail, IdentitySummary, RotateKeyResponse


class TestLoomalSupervisorClient:
    def test_requires_api_key(self):
        os.environ.pop("LOOMAL_SUPERVISOR_KEY", None)
        with pytest.raises(ValueError, match="Supervisor key is required"):
            LoomalSupervisor()

    def test_creates_with_api_key(self):
        client = LoomalSupervisor(api_key="mgsv-test123")
        assert client.identities is not None
        client.close()

    def test_reads_env_var(self):
        os.environ["LOOMAL_SUPERVISOR_KEY"] = "mgsv-fromenv"
        try:
            client = LoomalSupervisor()
            assert client.identities is not None
            client.close()
        finally:
            del os.environ["LOOMAL_SUPERVISOR_KEY"]

    def test_context_manager(self):
        with LoomalSupervisor(api_key="mgsv-test") as client:
            assert client.identities is not None


class TestAsyncLoomalSupervisorClient:
    def test_requires_api_key(self):
        os.environ.pop("LOOMAL_SUPERVISOR_KEY", None)
        with pytest.raises(ValueError, match="Supervisor key is required"):
            AsyncLoomalSupervisor()
...
class TestSupervisorIdentitiesResource:
    ...
    def test_create(self):
        ...
        client = LoomalSupervisor(api_key="mgsv-test")
    ...
    def test_sends_supervisor_key_in_header(self):
        ...
        client = LoomalSupervisor(api_key="mgsv-mysecret")
