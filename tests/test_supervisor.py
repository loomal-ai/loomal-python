...
import pytest
import respx
from mailgent import MailgentSupervisor, AsyncMailgentSupervisor
from mailgent.types import CreateIdentityResponse, IdentityDetail, IdentitySummary, RotateKeyResponse


class TestMailgentSupervisorClient:
    def test_requires_api_key(self):
        os.environ.pop("MAILGENT_SUPERVISOR_KEY", None)
        with pytest.raises(ValueError, match="Supervisor key is required"):
            MailgentSupervisor()

    def test_creates_with_api_key(self):
        client = MailgentSupervisor(api_key="mgsv-test123")
        assert client.identities is not None
        client.close()

    def test_reads_env_var(self):
        os.environ["MAILGENT_SUPERVISOR_KEY"] = "mgsv-fromenv"
        try:
            client = MailgentSupervisor()
            assert client.identities is not None
            client.close()
        finally:
            del os.environ["MAILGENT_SUPERVISOR_KEY"]

    def test_context_manager(self):
        with MailgentSupervisor(api_key="mgsv-test") as client:
            assert client.identities is not None


class TestAsyncMailgentSupervisorClient:
    def test_requires_api_key(self):
        os.environ.pop("MAILGENT_SUPERVISOR_KEY", None)
        with pytest.raises(ValueError, match="Supervisor key is required"):
            AsyncMailgentSupervisor()
...
class TestSupervisorIdentitiesResource:
    ...
    def test_create(self):
        ...
        client = MailgentSupervisor(api_key="mgsv-test")
    ...
    def test_sends_supervisor_key_in_header(self):
        ...
        client = MailgentSupervisor(api_key="mgsv-mysecret")
